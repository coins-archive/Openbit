...
#include "ui_zOpenbitcontroldialog.h"
...
std::list<std::string> ZOpenbitControlDialog::listSelectedMints;
std::list<CZerocoinMint> ZOpenbitControlDialog::listMints;

ZOpenbitControlDialog::ZOpenbitControlDialog(QWidget *parent) :
    QDialog(parent),
    ui(new Ui::ZOpenbitControlDialog),
    model(0)
{
    ui->setupUi(this);
    listMints.clear();
...
ZOpenbitControlDialog::~ZOpenbitControlDialog()
{
    delete ui;
}

void ZOpenbitControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void ZOpenbitControlDialog::updateList()
{
...
void ZOpenbitControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void ZOpenbitControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CZerocoinMint mint : listMints) {
    ...
    ui->labelZOpenbit_int->setText(QString::number(nAmount));
    ui->labelQuantity_int->setText(QString::number(listSelectedMints.size()));

    //update PrivacyDialog labels
    privacyDialog->setZOpenbitControlLabels(nAmount, listSelectedMints.size());
}

std::vector<CZerocoinMint> ZOpenbitControlDialog::GetSelectedMints()
{
...
void ZOpenbitControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);
