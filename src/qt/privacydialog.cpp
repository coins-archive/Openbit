...
#include "zOpenbitcontroldialog.h"
#include "spork.h"
                                                          ...
                                                          currentBalance(-1)
{
    ...
    // "Spending 999999 zOPN ought to be enough for anybody." - Bill Gates, 2017
    ui->zOPNpayAmount->setValidator( new QDoubleValidator(0.0, 21000000.0, 20, this) );
    ...
    ui->labelzOPNSyncStatus->setText("(" + tr("out of sync") + ")");
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        ui->pushButtonMintzOPN->setEnabled(false);
        ui->pushButtonMintzOPN->setToolTip(tr("zOPN is currently disabled due to maintenance."));

        ui->pushButtonSpendzOPN->setEnabled(false);
        ui->pushButtonSpendzOPN->setToolTip(tr("zOPN is currently disabled due to maintenance."));
    }
}
...
void PrivacyDialog::on_addressBookButton_clicked()
{
    ...
    if (dlg.exec()) {
        ...
        ui->zOPNpayAmount->setFocus();
    }
}

void PrivacyDialog::on_pushButtonMintzOPN_clicked()
{
    if (!walletModel || !walletModel->getOptionsModel())
        return;
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zOPN is currently undergoing maintenance."), QMessageBox::Ok,
                                 QMessageBox::Ok);
        return;
    }
    ...
    ui->TEMintStatus->setPlainText(tr("Minting ") + ui->labelMintAmountValue->text() + " zOPN...");
    ...
    QString strStatsHeader = tr("Successfully minted ") + ui->labelMintAmountValue->text() + tr(" zOPN in ") +
...
void PrivacyDialog::on_pushButtonSpendzOPN_clicked()
{
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zOPN is currently undergoing maintenance."), QMessageBox::Ok, QMessageBox::Ok);
        return;
    }
    ...
    if (encStatus == walletModel->Locked || encStatus == walletModel->UnlockedForAnonymizationOnly) {
        ...
        // Wallet is unlocked now, sedn zOPN
        sendzOPN();
        return;
    }
    // Wallet already unlocked or not encrypted at all, send zOPN
    sendzOPN();
}

void PrivacyDialog::on_pushButtonZOpenbitControl_clicked()
{
    ZOpenbitControlDialog* zOpenbitControl = new ZOpenbitControlDialog(this);
    zOpenbitControl->setModel(walletModel);
    zOpenbitControl->exec();
}

void PrivacyDialog::setZOpenbitControlLabels(int64_t nAmount, int nQuantity)
{
    ui->labelzOpenbitSelected_int->setText(QString::number(nAmount));
...
void PrivacyDialog::sendzOPN()
{
    ...
    if(ui->payTo->text().isEmpty()){
        ...
        if (!address.IsValid()) {
            QMessageBox::warning(this, tr("Spend Zerocoin"), tr("Invalid Openbit Address"), QMessageBox::Ok, QMessageBox::Ok);
            ui->payTo->setFocus();
            return;
        }
    }
    ...
    double dAmount = ui->zOPNpayAmount->text().toDouble();
    ...
    // Warn for additional fees if amount is not an integer and change as zOPN is requested
    ...
    if(!fWholeNumber && fMintChange){
        ...
        strFeeWarning += QString::number(dzFee, 'f', 8) + " OPN </b>will be added to the standard transaction fees!<br />";
    ...
    QString strAmount = "<b>" + QString::number(dAmount, 'f', 8) + " zOPN</b>";
    ...
    // use mints from zOpenbit selector if applicable
    vector<CZerocoinMint> vMintsSelected;
    if (!ZOpenbitControlDialog::listSelectedMints.empty()) {
        vMintsSelected = ZOpenbitControlDialog::GetSelectedMints();
    }
    ...
    // Clear zOpenbit selector in case it was used
    ZOpenbitControlDialog::listSelectedMints.clear();
    ...
    for (CZerocoinSpend spend : receipt.GetSpends()) {
        strStats += tr("zOpenbit Spend #: ") + QString::number(nCount) + ", ";
    ...
    for (const CTxOut& txout: wtxNew.vout) {
        strStats += tr("value out: ") + FormatMoney(txout.nValue).c_str() + " Openbit, ";
        nValueOut += txout.nValue;

        strStats += tr("address: ");
        CTxDestination dest;
        if(txout.scriptPubKey.IsZerocoinMint())
            strStats += tr("zOpenbit Mint");
                               ...
                               const CAmount& watchOnlyBalance, const CAmount& watchUnconfBalance, const CAmount& watchImmatureBalance)
{
    ...
    for (const auto& denom : libzerocoin::zerocoinDenomList) {
        ...
        strDenomStats = strUnconfirmed + QString::number(mapDenomBalances.at(denom)) + " x " +
                        ...
                        QString::number(nSumPerCoin) + " zOPN </b>";
    ...
    ui->labelzAvailableAmount->setText(QString::number(zerocoinBalance/COIN) + QString(" zOPN "));
    ui->labelzAvailableAmount_2->setText(QString::number(matureZerocoinBalance/COIN) + QString(" zOPN "));
    ui->labelzOPNAmountValue->setText(BitcoinUnits::floorHtmlWithUnit(nDisplayUnit, balance - immatureBalance - nLockedBalance, false, BitcoinUnits::separatorAlways));
}
...
{
    ui->labelzOPNSyncStatus->setVisible(fShow);
}
